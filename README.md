...
- **每小时抓取 RSS**（Python）
- **AI 分类**（漏洞 / 安全 / 忽略）与 **聚合摘要**
- **写入 Markdown**：
  - **今日**：`content/today.md`
  - **归档**：`content/archive/YYYY/MM/YYYY-MM-DD.md`
- **推送 Lark**：发送「新闻摘要 + 今日漏洞总结 + 数量统计」
...
| 路径 | 作用 |
| --- | --- |
| `vulnwatch/` | RSS 获取、去重、AI 分类/聚合、生成 Markdown、推送 Lark |
| `content/` | 输出目录：`today.md` 与 `archive/` |
| `site/` | Astro 前端站点：展示 today 与归档 |
| `config.yaml` | 配置（RSS/关键词/AI/Lark/输出路径） |
...
- **`ai`**：OpenAI 兼容接口（请求 `{base_url}/chat/completions`）
